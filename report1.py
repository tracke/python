...
def map_hwids(f):
	...
	grid_data1=[[int(start_time)+(t*60),[[''for x in range(len(hwids))] for y in range(len(hwids))]] for t in range(x_scale+1)]
	print(len(grid_data1))
	for i in range(x_scale+1):
		#print (grid_data[i])
		pass

	print("working with entries from ",start_time," to ",stop_time,"in ",x_scale,"steps")
	for x in hwids:				
		with open(f,'rt')as fp:
			data=csv.reader(fp)
			#print(hwids)
				...
				if any(row):
					if row[0] == 'source':
						...
						if row[2] == x:
							...
							#print(int(row[1])-t,visual)
							grid_data1[this_t][1][hwids.index(x)][hwids.index(row[0])]=row[5]
